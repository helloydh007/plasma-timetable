...
KDE Plasma 6 的桌面**课程表组件**（plasmoid / 桌面小组件），用来在桌面上看大学课表。
默认是 7 列 × 节次的**周网格**，也能切成「**今日课程 / 接下来 / 下一节**」三种列表样式
（列表还能再选卡片、紧凑行式、日程时间轴三种版式）。高亮今天与当前课程，
标出法定节假日与调休，背景和卡片的不透明度都能单独调。

课表可以**手动录入**，也可以**导入 ICS（iCalendar）**文件或**导入 WakeUp 课程表**的备份。
