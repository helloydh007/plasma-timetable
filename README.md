...
调休日到底补哪一天的课，**国家不规定，各校自己定**。所以配置页里为每个补班日提供一个下拉框：
按当天、按周一的课表、……、或者不排课。默认按当天本身的星期几。
...
覆盖 75 项断言：ICS 的折行/TZID/单双周/UNTIL/EXDATE、WakeUp 的 type 单双周与
...
所以凡是导入器会写到的字段（开学日、总周数、作息表）都在课程页；
调休安排因为要读节假日数据，单独存成 `workAsData` 归节假日页。
