...
| **接下来** | 从今天起往后几节，跨天标「明天」「后天」 | 想知道「接下来还有什么」 |
| **下一节** | 只显示当前或接下来那一节，大字加倒计时 | 占地方最小 |

正在上的那一节会用主题高亮色整块点亮（判据是「既是今天的、现在又落在它的起止时间之间」，
两个条件缺一不可 —— 只看时间的话，明天同一时间的课会被误判成正在上）。
已经上完的课不会列出来。

**「今日课程」有回退**：今天还有课就显示今天的；今天的都上完了，会自动接上最近的一节
（可能是明天），卡片上标出「明天 / 后天 / 周四」。一天课上完之后恰恰是最想知道
「下一节是什么」的时候，这时把组件空掉反而不合适。

面板上的小视图不受这里影响 —— 那里位置太窄，固定显示「现在 / 下一节」。

### 背景不透明度

「外观」页里可以调 0–100%。调到 0 就只剩内容浮在桌面上，完全没有背景板。
...
覆盖 201 项断言：ICS 的折行/TZID/单双周/UNTIL/EXDATE、WakeUp 的 type 单双周与
